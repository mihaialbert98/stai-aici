...
export default function NewRegistrationPage() {
  ...
  const saveSignature = async () => {
    if (!padRef.current || padRef.current.isEmpty()) {
      toast.error('Desenează semnătura înainte de a salva.');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) { toast.error('Eroare la salvarea semnăturii.'); return; }
      setSigPreview(sig);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!hasSignature) { toast.error('Salvează mai întâi semnătura recepționistului.'); return; }
    setSubmitting(true);
    try {
      ...
      if (!res.ok) { toast.error(data.error || 'Eroare la creare.'); return; }
    ...
    return (
      <div className="max-w-xl">
        <div className="card text-center py-10">
          ...
          <h2 className="text-xl font-bold mb-2">Formular creat!</h2>
          <p className="text-gray-500 mb-6 text-sm">
            Trimite link-ul de mai jos oaspeților tăi. Primul oaspete va introduce datele de cazare, iar ceilalți vor completa doar datele personale.
          </p>
  ...
  if (hasSignature === null) {
    return (
      <div>
        <h1 className="text-2xl font-bold mb-6">Fișă nouă de cazare</h1>
    ...
    <div className="max-w-xl">
      ...
      <div className="card mb-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-semibold">Semnătura recepționistului</h2>
        ...
        {hasSignature && !showSigPad ? (
          <div className="flex items-center gap-3">
            <img
              src={sigPreview!}
              alt="Semnătură recepționist"
              className="h-12 border border-gray-200 rounded-lg bg-white px-2"
            />
            <span className="text-sm text-green-600 font-medium">Semnătură salvată ✓</span>
          </div>
        ) : (
          <div>
            {hasSignature && (
              <p className="text-sm text-gray-500 mb-3">Desenează o semnătură nouă pentru a o înlocui pe cea existentă.</p>
            )}
            {!hasSignature && (
              <p className="text-sm text-gray-500 mb-3">
                Semnătura ta va apărea pe toate fișele de cazare generate. Adaug-o o singură dată.
              </p>
            )}
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <PenLine size={14} /> {savingSig ? 'Se salvează…' : 'Salvează semnătura'}
              </button>
              {hasSignature && showSigPad && (
                <button
                  type="button"
      ...
      <form onSubmit={handleSubmit} className={`card space-y-4 ${!hasSignature ? 'opacity-50 pointer-events-none' : ''}`}>
        <h2 className="font-semibold">Detalii formular</h2>

        {properties.length === 0 ? (
          <p className="text-sm text-gray-500">Nu ai proprietăți active. <a href="/dashboard/host/properties/new" className="text-primary-600 hover:underline">Adaugă una</a>.</p>
        ) : (
          <>
            <div>
            ...
            <div>
              <label className="label">Număr oaspeți *</label>
              <input
                type="number"
            ...
            >
              {submitting ? 'Se creează…' : 'Creează și obține link'}
            </button>
          </>
        )}
      </form>
